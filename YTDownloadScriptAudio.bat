...
:isNotLive
call:fancyText "Using link: %link% And converting into format: %format%", "Starting download using: yt-dlp -f bestaudio --merge-output-format %format% %link%"
yt-dlp -f bestaudio --merge-output-format %format% %link%
goto afterLive
...
:: fancy intro message 
:programIntro
	call:fancyText "Program created by Fred to download yt vods, this is AUDIO ONLY", "This specific one is made to download AUDIO ONLY, but i havent checked yet so eh"
	exit /b 0
